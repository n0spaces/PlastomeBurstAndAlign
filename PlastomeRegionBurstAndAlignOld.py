...
class Profiler:
    def __init__(self) -> None:
        self.timings: list[tuple[str, int, int]] = []
        self.last_fn_name = '(start)'
        self.last_dt = datetime.datetime.now()
    
    def next(self, fn_name: str) -> None:
        now = datetime.datetime.now()
        delta = now - self.last_dt
        self.last_dt = now
        self.timings.append((self.last_fn_name, delta.seconds, delta.microseconds))
        self.last_fn_name = fn_name
    
    def done(self, filepath: str) -> None:
        self.next('')
        # output timings in csv format
        with open(filepath, 'w') as f:
            f.write("function,seconds\n")
            f.writelines([f'{name},{s}.{us}\n' for name, s, us in self.timings])
...
def main(args):
    ...
    profiler.next("ExtractAndCollect")
    extractor = ExtractAndCollect(select_mode)
    profiler.next("ExtractAndCollect.conduct_extraction")
    main_odict_nucl, main_odict_prot = extractor.conduct_extraction(in_dir, fileext)

    profiler.next("DataCleaning")
    cleaner = DataCleaning(main_odict_nucl, main_odict_prot, select_mode)
    profiler.next("DataCleaning.remove_duplicate_annos")
    cleaner.remove_duplicate_annos()
    profiler.next("DataCleaning.remove_annos_if_below_minnumtaxa")
    cleaner.remove_annos_if_below_minnumtaxa(min_num_taxa)
    profiler.next("DataCleaning.remove_annos_if_below_minseqlength")
    cleaner.remove_annos_if_below_minseqlength(min_seq_length)
    profiler.next("DataCleaning.remove_orfs")
    cleaner.remove_orfs()
    profiler.next("DataCleaning.remove_user_defined_genes")
    main_odict_nucl, main_odict_prot = cleaner.remove_user_defined_genes(exclude_list)

    profiler.next("AlignmentCoordination")
    aligncoord = AlignmentCoordination(main_odict_nucl, main_odict_prot)
    profiler.next("AlignmentCoordination.save_regions_as_unaligned_matrices")
    aligncoord.save_regions_as_unaligned_matrices()
    if not select_mode == "cds":
        profiler.next("AlignmentCoordination.conduct_nucleotide_MSA")
        aligncoord.conduct_nucleotide_MSA()
    if select_mode == "cds":
        profiler.next("AlignmentCoordination.conduct_protein_MSA_and_backtranslate")
        aligncoord.conduct_protein_MSA_and_backtranslate()
    profiler.next("AlignmentCoordination.collect_successful_MSAs")
